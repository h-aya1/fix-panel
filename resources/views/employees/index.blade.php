...
.side-panel { 
    ...
    z-index: 1030; /* Lower than navbar's z-index */ 
...
#importExcelBtn { 
    background-color: #6c757d; /* Gray color to match the design */ 
    color: white; 
    border: none; 
    padding: 0.4rem 0.75rem; /* Match other buttons */ 
    border-radius: 0.2rem; /* Match other buttons */ 
    top: 1.5rem; /* Aligned with the top of the grid wrapper */ 
...
/* Styles for the static year and month display */ 
.date-display { 
    ...
    <div class="grid-wrapper"> 
        ...
        <div class="action-bar"> 
            ...
            <div class="right-actions"> 
                <button class="btn btn-outline-danger btn-sm" id="deleteSelectedBtn"><i class="bx bx-trash me-1"></i> Delete selected user</button> 
...
<div class="side-panel" id="allowanceDeductionPanel"> 
    <div class="panel-header"> 
        <h5 class="mb-0"> 
            <span id="panelEmployeeId"></span> 
            <span id="panelEmployeeName"></span> - Allowance & Deduction Details 
        </h5> 
...
$(document).ready(function() { 
        ...
        $.ajax({ 
            ...
            success: function(data) { 
                allEmployeeData = data.map(emp => ({ 
                    ...emp, 
                    company: emp.company_name, // Map company_name to company for display 
                    total_allowance: emp.qualification_allowance + emp.position_allowance + emp.duty_allowance + emp.overtime_allowance + emp.holiday_work_allowance + emp.night_shift_allowance + emp.bonus + emp.adjustment_allowance + emp.transportation_allowance + emp.meal_allowance + emp.labor_day_allowance + emp.paid_leave_allowance + emp.welfare_allowance + (emp.other_allowances || 0), 
                    total_deduction: emp.health_insurance + emp.long_term_care_insurance + emp.employment_insurance + emp.national_pension + emp.income_tax + emp.local_income_tax + (emp.other_deductions || 0), 
                    total_pay: (emp.base_salary || 0) + (emp.qualification_allowance + emp.position_allowance + emp.duty_allowance + emp.overtime_allowance + emp.holiday_work_allowance + emp.night_shift_allowance + emp.bonus + emp.adjustment_allowance + emp.transportation_allowance + emp.meal_allowance + emp.labor_day_allowance + emp.paid_leave_allowance + emp.welfare_allowance + (emp.other_allowances || 0)), 
                    net_pay: emp.net_payment || 0, 
                    remarks: emp.remarks || 'N/A', 
                    sms_sent: false, // Default to false since we don't have this field in EmployeeSummary 
                    employment_status: 'Working' // Default status 
                })); 
    ...
    $('#employeeGrid').on('cellclick', function (event) { 
        ...
        const columnData = $('#employeeGrid').jqxGrid('getcolumn', args.datafield);
        
        // Only proceed if clicking on Total Allowance or Total Deduction columns
        if (rowData && columnData && (columnData.datafield === 'total_allowance' || columnData.datafield === 'total_deduction')) {
            populateSidePanel(rowData); 
            $('#allowanceDeductionPanel').addClass('open');
            
            // Scroll to the relevant section based on which column was clicked
            if (columnData.datafield === 'total_allowance') {
                $('.panel-body').animate({
                    scrollTop: 0
                }, 300);
            } else {
                $('.panel-body').animate({
                    scrollTop: $('.details-section:eq(1)').offset().top - $('.panel-body').offset().top + $('.panel-body').scrollTop()
                }, 300);
            }
        } 
    }); 
    ...
    $('#deleteSelectedBtn').on('click', function() { 
        ...
        if (selectedIndexes.length === 0) { 
            showAlert('warning', 'Please select at least one employee to delete.'); 
            return; 
        } 
        const selectedIds = []; 
        selectedIndexes.forEach(index => { 
            const rowData = $('#employeeGrid').jqxGrid('getrowdata', index); 
            if (rowData) { 
        ...
        Swal.fire({ 
            title: 'Are you sure?', 
            text: `You are about to delete ${selectedIds.length} employee(s). This action cannot be undone!`, 
            icon: 'warning', 
            ...
            if (result.isConfirmed) { 
                $.ajax({ 
                    ...
                    success: function(response) { 
                        if (response.success) { 
                            showAlert('success', response.message || 'Selected employees have been deleted.'); 
                            loadEmployees(); 
                        } else { 
                    ...
                    error: function(xhr) { 
                        const errorMsg = xhr.responseJSON?.message || 'An error occurred while deleting employees.'; 
                        showAlert('danger', errorMsg); 
                    } 
                }); 
            } 
    ...
    function updateSmsPreview(employee) { 
        ...
        const blackTextContent = `<strong>[주]피에스엠씨] 급여 명세서 발송 2025년의 3월 급여가 지급되었습니다.</strong><br>` + 
                                `귀하의 노고에 감사드립니다.<br><br>` + 
                                `급여명세서 링크 확인하기<br>` + 
                                `www.google.com/jaden221`; 
        const blueTextContent = `[지급일] 2025년 04월 10일<br><br>` + 
    ...
    $('#fileInput').on('change', function(e) { 
        ...
        if (!validTypes.includes(file.type) && !file.name.match(/\.(xlsx?|csv)$/i)) { 
            showAlert('danger', 'Invalid file type. Please upload an Excel (.xlsx, .xls) or CSV file.'); 
        ...
        if (file.size > maxSize) { 
            showAlert('danger', 'File is too large. Maximum size allowed is 10MB.'); 
            $(this).val(''); // Clear the file input 
            return false; 
        } 
        ...
        const processingAlert = $('<div class="alert alert-info">Processing file, please wait...</div>') 
            .insertBefore(importBtn.parent()) 
            .hide() 
            .fadeIn(); 
        ...
        $.ajax({ 
            ...
            success: function(response) { 
                if (response.success) { 
                    const message = response.message || `Successfully imported ${response.imported_count || 0} employees.`; 
            ...
            error: function(xhr) { 
                let errorMsg = 'Error importing file'; 
                if (xhr.status === 422) { // Format validation errors 
                    const errors = xhr.responseJSON?.errors; 
                    if (errors) { 
                        errorMsg = 'Validation errors:<br>' + Object.values(errors).map(err => 
                            Array.isArray(err) ? err.join('<br>') : err 
                        ).join('<br>'); 
                    } else { 
                ...
                showAlert('danger', errorMsg); 
                console.error('Import error:', xhr.responseJSON || xhr.statusText); 
            }, 
            complete: function() { 
                importBtn.prop('disabled', false).html('Import Excel'); 
                // Reset file input 
                $('#fileInput').val(''); 
                // Remove processing message 
                processingAlert.fadeOut(function() { 
                    $(this).remove(); 
                }); 
            } 
        }); 
    }); 
    // Initialize click handler for import button 
